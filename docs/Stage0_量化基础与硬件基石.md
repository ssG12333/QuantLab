...
# f=1.0:     exp=0,   相邻间距=1.19e-7  (小值密)
# f=1000.0:  exp=9,   相邻间距=6.10e-5  (大值疏!)
# f=0.0001:  exp=-14, 相邻间距=7.28e-12 (非常密!)
...
1. **Round Error（舍入误差）**：`round()` 丢掉的零头。8-bit 下最大 `S/2 ≈ 0.014`。大批量中正负误差抵消，通常可控。

2. **Clip Error（截断误差）**：超出量程的值被硬夹住。**比 round error 严重得多**——一个 outlier 被 clip 可能改变整层输出的统计特性。校准算法的核心目标就是最小化 clip error。

```
Round Error  vs  Clip Error 的直觉:
  舍入误差: 每个元素最多损失 S/2 → 大批量中正负抵消 → 可控
  截断误差: 每个被 clip 的元素损失可能巨大 → 永不抵消 → 累积到后续层
              → 校准算法的核心就是"决定哪些值应该被 clip"
```
...
ReLU 后激活全在 [0, 3.0]。对称量化用 [-3, 3]→[-128, 127]，只用了 [0, 127]（50% 浪费）。
非对称把 [0, 3]→[0, 255]，全部用上。
