...
- [阶段 1.5：QAT 训练深度剖析 — 从"能跑"到"理解为什么"](#阶段-15qat-训练深度剖析--从能跑到理解为什么)
...
## 阶段 1.5：QAT 训练深度剖析 — 从"能跑"到"理解为什么"

> ⏱ 预计时间：2-3 周 | 🎯 难度：⭐⭐⭐
>
> Stage 1 教你"怎么调 QAT API"。这个 Stage 补上中间缺失的一环：**QAT 训练本身到底发生了什么。**
> 从手写 FakeQuantize 状态机开始，到观察不同比特下的"崩溃点"，到理解 LSQ 为什么是必然的下一步。
> 这一章是连接"API 使用者"和"算法设计者"的桥梁。

### 1.5.1 细粒度学习路径

```
Week 1: 手写 QAT — 理解每一个细节
  ├── 子路径 1: 从零构造 FakeQuantize（不用 torch.ao 的任何东西）
  │     ├── 手写 MinMaxObserver: register_buffer(min_val, max_val)
  │     ├── 手写 FakeQuantize: forward = round(x/s) * s（STE 自动处理！）
  │     ├── 手写 observer_enabled / fake_quant_enabled 两个开关
  │     └── 吃饭: 理解为什么 PyTorch 用两个 flag 而不是一个 mode 枚举
  │
  ├── 子路径 2: 手动管理三阶段状态切换
  │     ├── Phase 1 (Calibration): observer=T, fq=F → 收集 min/max
  │     ├── Phase 2 (QAT Training): observer=F, fq=T → scale 冻结, 训练 weight
  │     ├── Phase 3 (Evaluation): observer=F, fq=T, BN.eval() → 测试
  │     └── 动手: 在一个 2 层 MLP (MNIST) 上手写完整的 QAT 训练循环
  │
  ├── 子路径 3: 对比"手写 QAT"和"PyTorch prepare_qat_fx"的输出
  │     ├── 用两种方式对同一个 ResNet18 做 QAT
  │     ├── 对比每层的 scale/Z、weight 更新量、最终精度
  │     └── 吃饭: PyTorch 的 MovingAverageObserver 比手写 MinMax 好在哪
  │
  └── 子路径 4: BN 冻结的深层原理
        ├── 实验 A: 正常 QAT（BN 冻结） — 精度正常
        ├── 实验 B: BN 不冻结（继续更新 running stats）— 精度崩塌
        ├── 分析: 看 BN running_mean 的漂移曲线
        └── 吃饭: FakeQuant 改变了激活分布 → BN stats 被"污染"

Week 2: Observer 消融 + 比特宽度系统性实验
  ├── 子路径 5: Observer 类型大对决
  │     ├── MinMaxObserver: 最朴素, 对 outlier 极度敏感
  │     ├── MovingAverageObserver: EMA 平滑, 更稳定
  │     ├── HistogramObserver: KL/MSE 搜索最优截断
  │     ├── PerChannelObserver: 每通道独立 scale
  │     ├── 实验: 同一模型 × 同一比特 × 不同 Observer → 精度差异表
  │     └── 吃饭: 什么时候用哪个 Observer？（不是背，是理解）
  │
  ├── 子路径 6: 比特宽度系统性实验 —— 找到"崩溃点"
  │     ├── FP32 baseline → 8-bit → 6-bit → 4-bit → 3-bit → 2-bit
  │     ├── 记录每个比特的 test accuracy + 收敛 epoch 数
  │     ├── 观察"崩溃点": 在哪个比特数下 QAT 开始失效？
  │     └── 吃饭: 为什么 8-bit 几乎无损, 4-bit 开始降, 2-bit 直接崩？
  │
  ├── 子路径 7: QAT 损失曲面可视化
  │     ├── 沿着 FP32 模型的梯度方向行走，看 loss 如何变化
  │     ├── 对比 FP32 训练和 QAT 训练的 loss landscape
  │     └── 吃饭: FakeQuantize 在 loss 曲面上加了"台阶"（离散化）
  │
  └── 子路径 8: Observer 时序调度实验
        ├── 实验 A: observer 关闭太早（epoch 1）→ scale 不准
        ├── 实验 B: observer 关闭太晚（epoch 8）→ 浪费训练时间
        ├── 实验 C: 最佳时机（epoch 2-3）→ 精度最高
        └── 吃饭: 2-3 epoch 不是魔法数字——它是"统计稳定"和"训练效率"的平衡

Week 3: 失败案例 + 从"崩溃"到 LSQ 的桥梁
  ├── 子路径 9: QAT 失败案例分析
  │     ├── 案例 1: "QAT 精度比 PTQ 还差" → Observer 用了太短的 epoch
  │     ├── 案例 2: "loss 剧烈震荡" → BN 未冻结
  │     ├── 案例 3: "训练中 loss 突然上升" → LR 过大
  │     ├── 案例 4: "某些层输出全变成同一个值" → reduce_range 设错了
  │     └── 每个案例: 从"症状"→"根因"→"修复"的完整推理链
  │
  ├── 子路径 10: 固定 scale QAT 的能力边界
  │     ├── 在 2-bit 下深入分析: 为什么固定 scale QAT 会崩
  │     ├── 量化误差溯源: 多少来自 round error, 多少来自 clip error
  │     ├── 关键发现: 低比特下 clip error 主导 → scale 的精确性至关重要
  │     └── 吃饭: "如果 scale 能跟着 weight 一起学, 问题不就解决了？"
  │            ↑ 这就是 LSQ 的动机。学了 Stage 1.5 后, Stage 2 不再"硬着陆"
  │
  └── 子路径 11: 综合实验
        ├── 在 CIFAR-10 上, 用 ResNet-20, 做完整的 8/6/4/3/2-bit 对比
        ├── 画"比特数 vs 精度"曲线 — 找固定 scale QAT 的拐点
        └── 输出一份实验报告: 哪个比特数下哪个 Observer 最好

### 1.5.2 关键动手实验

| 实验 | 时间 | 产出 |
|------|:--:|------|
| 手写 FakeQuantize + QAT 训练循环 (MNIST) | 1h | 2 层 MLP, 8-bit QAT, >97% |
| Observer 消融 (4 种 Observer × 4 比特) | 1.5h | 精度对比表, 每个 Observer 的适用场景 |
| 比特宽度系统性实验 (8→6→4→3→2) | 1h | 找到"崩溃点", 画比特-vs-精度曲线 |
| BN 冻结 vs 不冻结对比 | 0.5h | running_mean 漂移曲线, 理解冻结必要性 |
| QAT 失败案例复现 + 修复 | 1h | 4 个案例的症状→根因→修复推理链 |

### 1.5.3 检验标准

- [ ] 能手写 FakeQuantize（observer + fake_quant + 两个开关），不用 torch.ao
- [ ] 能手写完整 QAT 训练循环（三阶段状态切换）
- [ ] 能画出 8/6/4/3/2-bit QAT 的精度曲线，指出"崩溃点"
- [ ] 能解释 4 种 Observer 的区别和使用场景
- [ ] 能诊断 4 种 QAT 失败案例（BN 未冻结/LR 过大/Observer 时间不对/reduce_range）
- [ ] 能解释为什么固定 scale QAT 在低比特下失效 → 引出 LSQ 的动机
...
```
阶段 0 (1-2周)     阶段 1 (2-3周)       阶段 1.5 (2-3周)       阶段 2 (2-3周)
  ┌─────────┐      ┌──────────┐        ┌──────────────┐      ┌─────────────┐
  │量化数学基础│ ──→ │PyTorch QAT│ ──→   │QAT 训练深剖  │ ──→  │LSQ 核心算法 │
  │+ 硬件原理 │      │三种模式   │        │手写→消融→崩溃 │      │+ 从零实现   │
  └─────────┘      └──────────┘        └──────────────┘      └─────────────┘
                                                                     │
                                          ┌───────────────────────────┘
                                          ↓
  阶段 3 (3-4周)      阶段 4 (2-3周)      阶段 5 (3-4周)        阶段 6 (3-4周)
  ┌───────────────┐  ┌──────────┐       ┌────────────┐       ┌──────────────┐
  │AdaRound/GPTQ  │  │YOLO QAT  │       │PPQ + AIMET │       │LLM PTQ 全家桶│
  │数学内核       │  │端到端实战 │       │工业级框架  │       │GPTQ/Smooth/  │
  └───────────────┘  └──────────┘       └────────────┘       │AWQ/SpinQuant │
        │                  │                   │              └──────────────┘
        └──────────────────┴───────────────────┘                      │
                                  │                                   │
                                  ↓                                   ↓
                            阶段 7 (3-4周)         阶段 8 (3-4周)    阶段 9 (追踪)
                            ┌──────────────┐      ┌────────────┐   ┌──────────┐
                            │LLM QAT       │ ──→  │端侧部署    │   │前沿拓展  │
                            │QLoRA/Efficient│     │ONNX→TRT→DLC│   │FP8/KV/...│
                            └──────────────┘      └────────────┘   └──────────┘
```
...
### 阶段 1.5
- [ ] 手写 FakeQuantize + QAT 训练循环（不用 torch.ao）
- [ ] 能画出 8/6/4/3/2-bit QAT 的精度曲线，指出"崩溃点"
- [ ] 能解释 4 种 Observer 的区别和适用场景
- [ ] 能诊断 4 种 QAT 失败案例
- [ ] 能解释为什么固定 scale QAT 在低比特失效 → LSQ 的动机
